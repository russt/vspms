export PROJECTRC   ; PROJECTRC=${PROJECTRC:-./projectrc.ksh}
...
{
	if [ -z $1 ]; then
		print "Usage:  delpj project_name"
		print "	deletes <project_name>"
		return 0
	fi
...
{
	if [ -z $1 ]; then
		print "Usage:  addpj project_name"
		print "	adds current directory as <project_name>"
		return 0
	fi
	print "$1	$PWD" >> $MYPROJECTS
	print "$1	$PWD"
}
...
function pushspj
{
	...
	if [ $# -gt 0 -a -d $PROJECT/$1 ]; then
		pushd $PROJECT/$1
		if [ $? -ne 0 ]; then
			print pushspj: $PROJECT/$SBPJ not accessible
			return 1
		fi
		edpjenv reset $sav $PROJECT $SBPJ
		SBPJ=$1
	...
	if [ ${tmp[0]} != $PROJECT ]; then
		PROJECT=${tmp[0]}
		SBPJ=${tmp[1]}
#print new pjenv PROJECT=$PROJECT SBPJ=$SBPJ
		edpjenv set $PWD $PROJECT $SBPJ
		...
		SBPJ=${tmp[1]}
#print restored pjenv PROJECT=$PROJECT SBPJ=$SBPJ
		edpjenv set $PWD $PROJECT $SBPJ
	fi
